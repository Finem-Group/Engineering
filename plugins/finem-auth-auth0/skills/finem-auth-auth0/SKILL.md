---
name: finem-auth-auth0
description: "Use when this project's work involves Auth0 integration only when the project has selected Auth0 — covers auth. Loads the original auth0 skills bundled with this pack."
---
...
- `auth0:auth0` → `upstream/auth0/plugins/auth0/skills/auth0/SKILL.md`

## Coordination

Finem's `finem-engineering` skill in `finem-core` owns capability selection, evidence and limits. Depends
on: `finem-core`.
Upstream sources bundled here: auth0. Their licenses and pinned revisions are recorded in
