# Finem React ui

shadcn component composition

**Claude Code**

```bash
claude plugin install finem-react-ui@finem
```

**Codex**

```bash
codex plugin add finem-core@finem
codex plugin add finem-react-ui@finem
```

**Depends on** `finem-core`. Claude Code installs those automatically; Codex does not resolve dependencies, so the Codex commands above install them in order.

| Capability | Role | Originals |
| --- | --- | --- |
| Frontend | adds to base | 1 |
| Design system | adds to base | 1 |

Native skills: 1. Bundled originals: 1 from 1 source repository (shadcn), unmodified under `upstream/`. See `NOTICE.md` for licenses and pinned revisions.
