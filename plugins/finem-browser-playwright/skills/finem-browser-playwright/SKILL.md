---
name: finem-browser-playwright
description: Use when this project's work involves Use Playwright as the browser execution backend — covers browser qa. Loads the original playwright skills bundled with this pack.
---
...
- `playwright:playwright-cli` → `upstream/playwright/skills/playwright-cli/SKILL.md`

## Coordination

Finem's `finem-engineering` skill in `finem-core` owns capability selection, evidence and limits. Depends
on: `finem-core`.
Upstream sources bundled here: playwright. Their licenses and pinned revisions are recorded in
