# Finem Backend fastapi

FastAPI routing, validation, dependency cleanup and tests

**Claude Code**

```bash
claude plugin install finem-backend-fastapi@finem
```

**Codex**

```bash
codex plugin add finem-core@finem
codex plugin add finem-backend-fastapi@finem
```

**Depends on** `finem-core`. Claude Code installs those automatically; Codex does not resolve dependencies, so the Codex commands above install them in order.

| Capability | Role | Originals |
| --- | --- | --- |
| Backend | base | 1 |
| API contracts | base | 1 |
| Testing | base | 1 |

Native skills: 1. Bundled originals: 1 from 1 source repository (fastapi), unmodified under `upstream/`. See `NOTICE.md` for licenses and pinned revisions.
